...
pub(crate) struct RssChannel
{
	...
	img_alt: String, // this is the img element's alt attribute; in practice it should match RssChannel.title
	img_tooltip: String, // this is the a element's title attribute, ie tooltip
	copyright: String,
	...
	#[serde(default)] PICS_rating: Option<String>,
...
{
	#[inline(always)]
	pub fn renderResource<'a, 'b: 'a, 'c>(&'c self, languageData: (&str, &Language), handlebars: &mut Handlebars, configuration: &Configuration, newResources: &'b mut Resources, oldResources: &Arc<Resources>, siteMapIndexUrls: &mut BTreeSet<Url>, items: &[RssItem], primary_iso_639_1_alpha_2_language_code: &str, iso_639_1_alpha_2_language_code: &str, resources: &'a BTreeMap<String, Resource>) -> Result<(), CordialError>
	{
		...
		{
			if let Some((url, response)) = xslStylesheet.urlAndResponse()
			{
				...
				{
					self.external_stylesheet_mime_type.parse()?
				};
		...
		eventWriter.writeWithinElement(Name::local("rss"), &namespace, &versionAttributes, |eventWriter|
		{
			eventWriter.writeWithinElement(Name::local("channel"), &namespace, &emptyAttributes, |eventWriter|
			{
				...
				eventWriter.writeUnprefixedTextElement(&namespace, &emptyAttributes, "link", languageData.1.baseUrl(languageData.0).as_ref())?;
				...
				{
					eventWriter.writeUnprefixedTextElement(&namespace, &emptyAttributes, "url", IMAGE_URL)?;
					eventWriter.writeUnprefixedTextElement(&namespace, &emptyAttributes, "title", &self.img_alt)?;
					eventWriter.writeUnprefixedTextElement(&namespace, &emptyAttributes, "description", &self.img_title)?;
					...
					eventWriter.writeUnprefixedTextElement(&namespace, &emptyAttributes, "width", &format!("{}", IMAGE_WIDTH))?;
					eventWriter.writeUnprefixedTextElement(&namespace, &emptyAttributes, "height", &format!("{}", IMAGE_HEIGHT))?;
				})?;
				
				if let Some(ref rating) = self.PICS_rating
				{
					eventWriter.writeUnprefixedTextElement(&namespace, &emptyAttributes, "rating", rating.to_str())?;
				}
				// textInput, skipHours and skipDays are not generated
				Ok(())
			})
		})?;
		...
		let mut bodyUncompressed = eventWriter.into_inner().bytes();
	...
	#[inline(always)]
	fn createEventWriter<'a>() -> EventWriter<LengthTrackingWriter<'a>>
	{
